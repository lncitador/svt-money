...
export class ApiService<T = never, R = T> {
    ...
    public async getById(id: ID, queryParams?: QueryProps, v?: string): Promise<AxiosResponse<R, any>>
    ...
    public async getById(id: ID, arg1?: QueryProps | string, arg2?: string) {
        ...
        if (!arg1) {
            ...
            const params = mountUrl('', arg1 as QueryProps);
    ...
    public async put(id: ID, data: Partial<T>, v = 'v1') {
        return axiosInstance.put<T, R>(
            `${v}/${this.DOMAIN_REF}/${id}`,
            data as T,
        );
    }

    public async patch(id: ID, data: Partial<T>, v = 'v1') {
        return axiosInstance.patch<T, R>(
            `${v}/${this.DOMAIN_REF}/${id}`,
            data as T,
        );
    }
